...
1. What are the main differences between a stateful and a functional component?
   `Stateful components store data through an object which re-renders when changed. Functional components simply render JSX, though a state hook can be used.`

2. When does a componentWillMount function be called? What about a componentWillUpdate?
   `componentWillMount is called before the component renders in order to prepare for the first render. componentWillUpdate is called whenever we need to re-render and update state.`

3. Define stateful logic.
   `Any code that uses state, which is stored data that can be updated through events.`

4. What are the three steps of creating a successful test? What is done in each phase?
   `Arrange- Initialize objects and sets values Act- Invokes methods Assert- Verify the test outcomes pass expectations `

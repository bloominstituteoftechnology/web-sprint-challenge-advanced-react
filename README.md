...
component ,mounting phase and the initial data is set up and invoked with render being the view as a function of state set up through a constructor. Update phase comes second. this is where the state is changed through set.state and then it re-renders. Third stage is unmounting phase where the component is removed from the Dom and then cleanup takes place.
...
Testing makes sure that our app is working as intended. To successfully test your app you need three things. Arrange, which checks that there is an element. Act, which makes a change or fire an event and finally assert, which makes sure that it exists.

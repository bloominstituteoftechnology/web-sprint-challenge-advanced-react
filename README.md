...
1). Mounting: to put elements in the DOM.
2). Updating: this is done whenever the components state or props change.
3). UnMounting: when react removes a component from the DOM
...
3). Act: query the DOM
4). Assert: what we expect to happen
5). Run Test
...
- [x] In the `PlantList` class component, fetch data from the server you now have running - the data can be fetched from `npm`
